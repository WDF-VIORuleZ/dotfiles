...
export ZSH="/home/mxhdrm/.oh-my-zsh"
...
path+=(/home/mxhdrm/.bin/)
path+=(~/.dat/releases/dat-13.12.2-linux-x64)
path+=(~/.local/bin)
export path
...
LS_COLORS='no=00:*.exe=05;46:*.cc=33:*.cpp=33:*.hh=33:*.hpp=33'
...
alias ghidra=/opt/ghidra_9.0/ghidraRun --force-device-scale-factor=1.5 %U
alias m_pac="sudo pacman"
alias m_html2pdf="/usr/bin/wkhtmltopdf"
alias m_sp="spotify --force-device-scale-factor=2 %U"

alias m_sh="cd ~/Documents/programming/bash_scripts-git/bash_scripts-git"
alias m_src="cd ~/Documents/programming"
alias m_ctf="cd ~/Documents/ctf"
alias m_flx="cd ~/Documents/ctf/flux_fingers_rookies"
alias m_itsgit="cd ~/Documents/uni/RUB_ITS/sem_2"

# link to current semester
function sem { cd ~/Documents/uni/semester_$1; }

# ida wine wrapper
function m_ida32 { wine "/home/mxhdrm/Documents/proc/IDA 7.0/ida.exe" $1 }
function m_ida64 { wine "/home/mxhdrm/Documents/proc/IDA 7.0/ida64.exe" $1 }

# m_dp = m_diskspace: return disk usage in directory
function m_dp { l | awk {'print $2'} | grep -E '\w[KMG]{1}' 2>&/dev/null }
function m_server_dump 
{
    if [[ $# -eq 4 ]];then
        wget --recursive --no-parent --user $1 --password $2 $3
    elif [[ $# -eq 1 ]];then
        wget --recursive --no-parent $1
    else
        printf "[-] m_server_dump [username] [password] [url]\n"
    fi
}
# Exports
...
# Custom functions
function m_onboot() { sudo pacman-mirrors --country Germany --api --set-branch unstable --protocol https && sudo pacman -Syyu }
